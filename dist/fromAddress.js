...
export async function buildActionsFromAddress(input) {
    const { rpc, address, total, pageSize, before, until, fromDate, toDate, debug, windowTotalFromOut, requireAuthorityUserForOut, } = input;
    if (!address)
        throw new Error("buildActionsFromAddress: address is required");
    ...
    while (!done) {
        ...
        for (let i = 0; i < sigPage.length; i += CHUNK) {
            ...
            for (let j = 0; j < sigChunk.length; j++) {
                ...
                if (hasDateFilter) {
                    if (upperTs && blockTime > upperTs)
                        continue; // too recent
                    if (lowerTs && blockTime < lowerTs)
                        continue; // too old
                }
                try {
        ...
        if (!target && sigPage.length < perPage)
            break;
    }
    ...
    for (const ctx of parsed) {
        ...
        try {
            ...
            if (target && allActions.length >= target)
                break;
        }
        catch (e) {
...
//# sourceMappingURL=fromAddress.js.map