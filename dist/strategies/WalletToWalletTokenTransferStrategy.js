 ...
 */
export class WalletToWalletTokenTransferStrategy {
    constructor() {
    ...
    match(edges, userTokenAccounts, userWallet, opts) {
        ...
        const dbg = (...args) => {
            if (debug)
                log("[WalletToGatewayTransfer]", ...args);
        };
        const legs = [];
        // Step 1: collect candidate user-out edges (excluding WSOL)
        const userOuts = edges.filter((e) => tags?.get(e.seq) === "normal" &&
...
//# sourceMappingURL=WalletToWalletTokenTransferStrategy.js.map