 ...
 */
export function pushUserSolDeltaEdge(tx, edges, userWallet, opts) {
    const dustLamports = opts?.dustLamports ?? 500;
    const msg = tx?.transaction?.message;
    const meta = tx?.meta;
    if (!msg || !meta || !userWallet)
        return;
    ...
    if (Math.abs(residualLamports) <= dustLamports)
        return; // ignore dust-level deltas
    // Create synthetic edge
    const incoming = residualLamports > 0;
    const nextSeq = Math.max(-1, ...edges.map((e) => (typeof e.seq === "number" ? e.seq : -1))) + 1;
    edges.push({
        seq: nextSeq,
...
//# sourceMappingURL=solDeltas.js.map