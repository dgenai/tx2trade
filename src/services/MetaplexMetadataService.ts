 ...
 */
function toMetadataPda(mint: string): string {
...
export class MetaplexMetadataService {
  constructor(private rpc: SolanaRpcClient) {}
   ...
   */
  enrichActionsWithMetadata<T extends TradeAction>(
    actions: T[],
    metaMap: Record<string, TokenMeta>
  ): T[] {
    return actions.map((a) => {
      // Deep clone to avoid mutating original actions
      const out: T = JSON.parse(JSON.stringify(a));
      ...
      if (out.sold?.address) {
        ...
        if (mint === WSOL_MINT) {
          ...
          if (name) out.sold.name = name;
          if (symbol) out.sold.symbol = symbol;
          if (uri) out.sold.uri = uri;
        }
      }
      ...
      if (out.bought?.address) {
        ...
        if (mint === WSOL_MINT) {
          ...
          const { name, symbol, uri } = metaMap[mint];
          if (name) out.bought.name = name;
          if (symbol) out.bought.symbol = symbol;
          if (uri) out.bought.uri = uri;
        }
      }
