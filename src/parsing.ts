...
export type ParsedAction = any; // replace with your concrete Action type
export type ParsedTx = { sig: string; tx: any | null };
 ...
 */
export function parseTransactionToActionsWithoutCandles(
  sig: string,
  tx: any,
  options: ParseOptions
): { actions: ParsedAction[]; blockTime: number | null } {
  const { debug, windowTotalFromOut, requireAuthorityUserForOut } = options;

  if (!tx) {
    if (debug) console.warn(`tx2trade: missing transaction: ${sig}`);
    return { actions: [], blockTime: null };
  }

  if (tx.meta?.err) {
    if (debug) console.warn(`tx2trade: skip failed transaction: ${sig}`);
    return { actions: [], blockTime: null };
  }
