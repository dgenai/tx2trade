 ...
 *  - Known PDAs
 */
export function inferUserWallets(tx: any): string[] {
  const wallets = new Set<string>();

  const accountKeys = tx?.transaction?.message?.accountKeys ?? [];

  function to58(k: any): string {
    if (!k) return "";
    if (typeof k === "string") return k;
  ...
  const addOwner = (tb: any[]) => {
    for (const e of tb ?? []) {
      const o = e?.owner;
      if (o && signers.has(o)) wallets.add(o);
    }
  };
  ...
  // 3. ATA creations
  // -----------------------------
  function collectATA(ix: any) {
    if (ix?.parsed?.type === "create" || ix?.parsed?.type === "createIdempotent") {
      const w = ix.parsed?.info?.wallet;
      if (w && signers.has(w)) wallets.add(w);
    }
  }

  for (const ix of tx?.transaction?.message?.instructions ?? []) collectATA(ix);
  for (const inner of tx?.meta?.innerInstructions ?? [] )
    for (const ix of inner?.instructions ?? []) collectATA(ix);
  ...
  for (const w of [...wallets]) {
    if (!w || blacklist.has(w)) wallets.delete(w);
  }
