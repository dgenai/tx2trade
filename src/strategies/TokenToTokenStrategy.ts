...
export class TokenToTokenStrategy implements LegStrategy {
  ...
  match(
    ...
    const path: TransferEdge[] = [];

    for (const e of edges) {
      if (e.mint === WSOL_MINT) continue; // kept out of this strategy
    ...
    const soldEdgeForWallet = path.find(
      (e) =>
        e.mint === soldMint &&
        userWallets.includes(e.authority ?? "")
    ) ?? path.find(
    ...
    const userWallet =
      (soldEdgeForWallet && userWallets.find((w) => w === soldEdgeForWallet.authority)) ||
      userWallets[0] ||
      "";
